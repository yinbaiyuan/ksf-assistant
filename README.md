...
KSFAssistant 只负责飞书消息与任务卡片通信：机器人发送和更新卡片，已绑定的远程操作者提交输入及控制任务。首次接入未取得本人标识时，临时申请最小身份权限用于绑定；不为 Agent 业务操作管理用户授权。

Agent 操作文档、日历、审批等业务时使用独立工具、独立应用及独立凭据。助手不代理这些命令，不安装或发布飞书业务 Skills，也不展示业务批准窗口。任务消息与卡片只通过产品内置的官方 SDK/OpenAPI 通道工作。

旧受管入口通过随包迁移命令 `ksf-assistant-toolchain retire --json` 退役：只处理所有权及哈希校验通过的旧安装，保留事务备份；用户修改或非本产品文件拒绝覆盖。旧飞书命令入口变为明确拒绝执行的占位程序，KSF 任务上报入口保留。迁移不读取、复制或清除独立 CLI 的凭据。
