...
- **Task Feishu link:** Each real task row may expose one compact paper-plane control after the task-detail control. The task-detail page repeats the same connect/release action and adds one quiet status stack: authorized alias plus `全权限 · 24 小时`, current turn state/controller, remaining lease/last sync, and a bounded phase summary. Never display thread, turn, message, or real Feishu identifiers. Keep the home row compact; progress cards, remote controls, attachments and input remain bridge-owned. Card forms keep the input and its submit action on the first row, then place mode, stop, close, or implementation controls in an 8px-separated right-aligned action row; button groups retain 8px internal spacing. A completed Plan awaiting implementation is a distinct orange `等待开始执行` state: preserve the plan body, label the input `修改计划`, show one blue primary `开始执行` action, use a secondary `提交修改` submit action, keep `断连`, and hide the ordinary next-turn mode switch. Failed starts keep the same plan and primary action with one concise retry explanation.
