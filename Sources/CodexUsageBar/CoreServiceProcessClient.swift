...
enum CoreServiceError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .executableMissing:
            return "未找到 CodexAssistant 核心服务。"
        case .processStopped:
            return "CodexAssistant 核心服务已停止。"
        case let .pipeReadFailed(message):
            return "读取 CodexAssistant 核心服务失败：\(message)"
        case .invalidResponse:
...
actor CoreServiceProcessClient {
    ...
    func start(ksfRoot: String) throws {
        ...
        if let runtime = Self.locateFeishuRuntime() {
            ...
            environment["CODEX_USAGE_BAR_FEISHU_BRIDGE"] = runtime.bridge.path
            environment["CODEX_USAGE_BAR_LARK_CLI"] = runtime.larkCLI.path
