...
target_name="${INSTALL_APP_NAME:-Codex Usage Bar.app}"
...
if [[ -d "$default_target" ]]; then
    ...
    if [[ "$existing_identifier" != "com.ksf.codexusagebar" && -z "${INSTALL_APP_NAME:-}" ]]; then
        target_name="Codex Usage Bar Team.app"
    fi
fi
...
/usr/bin/lipo -info "$target_app/Contents/MacOS/CodexUsageBar"
