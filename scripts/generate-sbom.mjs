...
const packages = [{
  SPDXID: 'SPDXRef-Application', name: 'CodexAssistant', versionInfo: '0.10.0-preview.1',
  downloadLocation: 'NOASSERTION', filesAnalyzed: false, licenseConcluded: 'MIT', licenseDeclared: 'MIT',
}];
for (const module of goModules.filter((item) => item.Path !== 'codexusagebar/core')) {
  packages.push({
...
const document = {
  ...
  name: `CodexAssistant-${revision.slice(0, 12)}`,
...
writeFileSync(path.join(output, 'CodexAssistant.spdx.json'), `${JSON.stringify(document, null, 2)}\n`);
