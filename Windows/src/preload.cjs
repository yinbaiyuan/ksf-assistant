...
contextBridge.exposeInMainWorld('ksfAssistant', Object.freeze({
  onDashboard: (callback) => {
    ...
    ipcRenderer.on('ksfassistant:dashboard', listener);
    return () => ipcRenderer.removeListener('ksfassistant:dashboard', listener);
  },
  ...
  clearDirectory: (kind) => ipcRenderer.invoke('directory:clear', kind),
  ...
  openWorkspacePath: (workspaceId, targetPath) => ipcRenderer.invoke('workspace:path-open', { workspaceId, targetPath }),
  ...
  createWorkspaceTask: (payload) => ipcRenderer.invoke('workspace:task-create', payload),
