...
import { browserLimit, decision, decisionLabels, riskLabels, parseFields, terminalStates } from '../../shared/model.mjs';
import JsonView from './JsonView.vue';
import ConfirmAction from './ConfirmAction.vue';

const props = defineProps({ capability: Object, policy: Object, seed: Object, latestRecord: Object });
const emit = defineEmits(['result', 'save-case']);
const values = reactive({});
const files = reactive({});
const uploads = reactive({});
const uploadGenerations = new Map();
const loadingFiles = computed(() => Object.keys(uploads).length > 0);
const mode = ref('form');
const raw = ref('{}');
const error = ref('');
const busy = ref('');
...
const limitation = computed(() => browserLimit(props.capability));
const invalidates = () => { validation.value = null; acknowledge.value = false; };
watch(() => props.latestRecord, row => {
  if (!row?.operationId || row.operationId !== operation.value?.id) return;
  if (row.status === 'outcome_unknown' && !row.response?.operation) {
    result.value = { status: 'outcome_unknown', unconfirmedOperationId: row.operationId, note: '运行记录中的调用响应未确认，不代表服务终态；请先查询原操作，不要重复确认。' };
  } else if (row.response?.operation && row.status !== operation.value.status) {
    result.value = { ...row.response, note: '状态已由运行记录同步；完整结果请查询原操作。' };
  }
});
...
async function upload(name, event) {
  const generation = (uploadGenerations.get(name) || 0) + 1;
  uploadGenerations.set(name, generation);
  delete files[name]; delete uploads[name]; invalidates(); error.value = '';
  ...
  try {
    ...
    if (uploadGenerations.get(name) !== generation) return;
    let encoded = '';
    ...
    if (uploadGenerations.get(name) === generation) error.value = '附件读取失败，请重新选择文件；旧附件已清除。';
  } finally { if (uploadGenerations.get(name) === generation) delete uploads[name]; }
}

async function act(action) {
  if (busy.value || loadingFiles.value) return;
  error.value = ''; busy.value = action;
  const start = performance.now();
  const capabilityId = props.capability.id;
  const originalOperationId = operation.value?.id || result.value?.unconfirmedOperationId;
  let requestId;
  try {
    if (action === 'validate') {
      ...
      if (action === 'execute') {
        ...
        requestId = body.requestId; result.value = null; invalidates();
        next = await api('execute', body);
      } else if (action === 'status') next = await query('operation', { id: originalOperationId });
      else {
      ...
      emit('result', { time: Date.now(), capabilityId, requestId, status: next.operation?.status || next.status, operationId: next.operation?.id, elapsedMs: Math.round(performance.now() - start), response: result.value });
    }
  } catch (issue) {
    error.value = issue.message;
    if (!['validate', 'status'].includes(action)) {
      const operationId = action === 'execute' ? undefined : originalOperationId;
      result.value = { status: issue.code || 'failed', requestId, unconfirmedOperationId: operationId, note: issue.message };
      emit('result', { time: Date.now(), capabilityId, requestId, operationId, status: result.value.status, elapsedMs: Math.round(performance.now() - start) });
    }
  } finally { busy.value = ''; }
}
...
<template>
  <section class="workbench" aria-label="能力测试工作区" :aria-busy="!!busy">
    ...
    <p v-if="limitation" class="notice warning" role="status">{{ limitation }}</p>
    ...
    <p v-if="loadingFiles" class="notice" role="status">附件读取中，完成后需要重新校验。</p>
    ...
    <div class="action-row"><button :disabled="!!busy || loadingFiles || !!limitation" @click="act('validate')">{{ busy === 'validate' ? '校验中…' : '校验参数' }}</button><button class="primary" :disabled="!!busy || loadingFiles || !!limitation || !proofValid || !acknowledge || ['disabled', 'unknown'].includes(permission.value) || pending" @click="act('execute')">{{ busy === 'execute' ? '执行中…' : permission.value === 'confirm_each' ? '创建待确认操作' : '执行能力' }}</button><button class="quiet" :disabled="!!busy" @click="saveCase">存为会话用例</button></div>
    ...
    <section v-if="result" class="result-section" aria-live="polite">
      ...
      <p v-if="result.requestId" class="hint">本机请求标识：<code>{{ result.requestId }}</code>（不是 Operation ID；服务未返回 ID 时请先核对审计。）</p>
      <button v-if="result.unconfirmedOperationId" :disabled="!!busy" @click="act('status')">查询原操作状态</button>
      <template v-if="operation"><code class="full-id">{{ operation.id }}</code><p>{{ operation.summary }}</p><p v-if="operation.errorCode" class="notice error">{{ operation.errorCode }}</p><p v-if="operation.nextAction" class="hint">下一步：{{ operation.nextAction }}</p>
        <div v-if="operation.status === 'awaiting_confirmation'" class="notice warning"><strong>等待本机逐次确认</strong><p>不是飞书审批流。请核对上方 Operation 摘要；{{ timeLabel(operation.challengeExpiresAt) }} 后过期。</p><div class="action-row"><ConfirmAction primary label="确认并执行" :disabled="!!busy || !result.challenge" :message="`${operation.id}：${operation.summary}。可能立即产生真实副作用。`" @confirm="act('confirm')" /><ConfirmAction label="取消操作" :disabled="!!busy" :message="`取消 ${operation.id}；已经提交远端的动作不保证撤销。`" @confirm="act('cancel')" /></div></div>
        <p v-if="operation.status === 'outcome_unknown'" class="notice warning">结果未知不等于失败。先核对远端及审计，不能自动重试。</p>
        <div class="action-row"><button :disabled="!!busy" @click="act('status')">查询原操作状态</button><ConfirmAction v-if="pending && operation.status !== 'awaiting_confirmation'" label="请求取消" :disabled="!!busy" :message="`取消 ${operation.id}；已经提交远端的动作不保证撤销。`" @confirm="act('cancel')" /></div>
      </template>
