...
async function read() {
  ...
  try {
    data.value = await query(kind, kind === 'operation' ? { id: id.value.trim() } : kind === 'recent' ? { box: box.value, limit: limit.value } : { box: box.value, id: id.value.trim() });
    if (kind === 'operation' && data.value.operation?.id) {
      const previous = props.history.find(row => row.operationId === data.value.operation.id);
      if (previous) emit('result', { ...previous, time: Date.now(), status: data.value.operation.status, response: { ...data.value, challenge: previous.response?.challenge } });
    }
  }
...
async function approval(row, action) {
  ...
  try {
    ...
    if (issue.code === 'outcome_unknown') {
      data.value = { status: 'outcome_unknown', operationId: row.operationId, note: issue.message };
      emit('result', { ...row, time: Date.now(), status: 'outcome_unknown', response: undefined });
    }
  }
  finally { busy.value = false; }
}
...
<template>
  <section class="page-section"><div class="section-heading"><div><h2>运行记录</h2><p class="muted">当前页面会话 · 不自动保存输入、正文或 challenge</p></div><div class="action-row"><button :disabled="!history.length" @click="downloadReport(reportRows(props.history))">导出脱敏摘要</button><button :disabled="!history.length" @click="emit('clear')">清空本页记录</button></div></div>
    ...
    <section v-for="row in history.filter(item => item.status === 'awaiting_confirmation')" :key="row.operationId" class="notice warning"><h3>待逐次确认 · {{ row.capabilityId }}</h3><code>{{ row.operationId }}</code><p>{{ row.response?.operation?.summary }}</p><p>有效期至 {{ timeLabel(row.response?.operation?.challengeExpiresAt) }}，以服务最新状态为准。</p><div class="action-row"><ConfirmAction label="确认并执行此操作" :disabled="busy || !row.response?.challenge" :message="`${row.operationId}：${row.response?.operation?.summary}。请核对目标，可能立即产生真实副作用。`" @confirm="approval(row, 'confirm')" /><ConfirmAction label="取消此操作" :disabled="busy" :message="`取消 ${row.operationId}；已经提交远端的动作不保证撤销。`" @confirm="approval(row, 'cancel')" /></div></section>
