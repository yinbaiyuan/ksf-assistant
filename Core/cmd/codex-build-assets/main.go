...
func writeSBOM(repoRoot string) error {
	...
	packages := []spdxPackage{{SPDXID: "SPDXRef-Application", Name: "CodexAssistant", VersionInfo: "0.10.0-preview.1", DownloadLocation: "NOASSERTION", FilesAnalyzed: false, LicenseConcluded: "MIT", LicenseDeclared: "MIT"}}
	for _, module := range modules {
		if module.Path == "codexusagebar/core" {
			continue
		}
	...
	document := map[string]any{
		...
		"name":              "CodexAssistant-" + revision[:min(12, len(revision))],
		"documentNamespace": "https://codexassistant.invalid/spdx/" + revision,
		"creationInfo":      map[string]any{"created": time.Unix(0, 0).UTC().Add(seconds).Format(time.RFC3339), "creators": []string{"Tool: codex-build-assets"}},
	...
	data = append(data, '\n')
	if err := os.WriteFile(filepath.Join(outputDir, "CodexAssistant.spdx.json"), data, 0o644); err != nil {
		return err
	}
