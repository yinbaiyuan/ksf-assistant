...
func TestReleaseCardActionAcceptsAlreadyReleasedReplay(t *testing.T) {
	...
	value := string(source)
	if !strings.Contains(value, `case "task_link_release":`) || !strings.Contains(value, `link.LinkState == "released"`) || !strings.Contains(value, `runtime.links.ReleaseByID(link.ID)`) {
...
func TestTaskLinkAnswerRejectsStaleQuestionBeforeMutatingCard(t *testing.T) {
	link := feishu.TaskLink{Extra: map[string]json.RawMessage{}}
...
func TestProjectDesktopTaskLinkTracksRunningDesktopTurn(t *testing.T) {
	...
	link := feishu.TaskLink{TurnState: "completed", TurnOwner: "none", ActionRequired: "none"}
...
func TestProjectDesktopTaskLinkTracksCompletedResultOnce(t *testing.T) {
	...
	link := feishu.TaskLink{TurnState: "completed", TurnOwner: "none", ActionRequired: "none", Phase: "已完成", Detail: "修复已经完成。", Extra: map[string]json.RawMessage{}}
...
func TestWaitingInputRequestIdentityTriggersCardSynchronization(t *testing.T) {
	...
	link := feishu.TaskLink{TurnState: "waiting_input", TurnOwner: "desktop", ActionRequired: "feishu", ActiveTurnID: "turn-1", Phase: "等待输入", Detail: "Codex 等待你的选择。", Extra: map[string]json.RawMessage{}}
	link.SetExtraString("pendingQuestionRequestID", "request-old")
	link.SetExtraString("pendingQuestionRevision", feishu.PendingQuestionRevision("turn-1", "request-old", questions))
	next := desktopTaskProjection{
		...
		PendingRequestID: "request-new", PendingQuestionRevision: feishu.PendingQuestionRevision("turn-1", "request-new", questions),
	}
