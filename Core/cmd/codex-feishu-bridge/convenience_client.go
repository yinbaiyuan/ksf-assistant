...
func runConvenienceClient(dataRoot string, settings feishu.Settings, service *feishu.CapabilityService, family string, arguments []string, write clientJSONWriter) error {
	...
	if clientOptionalFlag(flagArguments, "--payload-file") != "" {
		payload, err := clientPayload(flagArguments)
		if err != nil {
			return err
		}
	...
	if err := applyCompatibilityPrivateInputs(family, arguments, flagArguments, input); err != nil {
		return err
	}
	...
	if len(spec.Additional) > 0 {
		...
		primary, prepareErr := service.Prepare(context.Background(), spec.CapabilityID, input, "codex")
		if prepareErr != nil {
			return prepareErr
		}
		...
		for _, label := range labels {
			...
			additional, additionalErr := service.Prepare(context.Background(), capabilityID, filterCompatibilityInput(additionalDefinition, input), "codex")
	...
	result, err := service.Prepare(context.Background(), spec.CapabilityID, input, "codex")
	if err != nil {
		return err
	}
	if result.Submitted {
...
func applyCompatibilityPrivateInputs(family string, arguments, flags []string, input map[string]any) error {
	if family == "message" && len(arguments) > 0 && arguments[0] == "thread" {
		value, err := clientPrivateValue(flags, "--message-id-file")
		if err != nil {
			return err
		}
	...
	if family == "knowledge" && len(arguments) > 1 && arguments[0] == "comments" && arguments[1] == "add" {
		value, err := clientPrivateValue(flags, "--content-file")
		if err != nil {
			return err
		}
