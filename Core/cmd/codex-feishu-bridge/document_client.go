...
func runDocumentClient(dataRoot string, service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	if len(arguments) < 1 || arguments[0] != "create" && arguments[0] != "update" {
		return errors.New("doc action must be create or update")
	}
	flags := arguments[1:]
	content, err := clientPrivateValue(flags, "--content-file")
	if err != nil {
		return err
	}
	...
	if arguments[0] == "update" {
		...
		if mode == "" || mode == "append" {
			...
			if mode == "str_replace" {
				pattern, patternErr := clientPrivateValue(flags, "--pattern-file")
				if patternErr != nil {
					return patternErr
				}
	...
	prepared, err := service.Prepare(context.Background(), capabilityID, input, config.DefaultSource)
	if err != nil {
