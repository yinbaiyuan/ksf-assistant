...
func runDirectoryClient(settings feishu.Settings, capabilities *feishu.CapabilityService, kind feishu.DirectoryKind, action string, arguments []string, store feishu.ClientConfigStore, write clientJSONWriter) error {
	...
	case "search":
		...
		matches, err := service.Search(context.Background(), kind, query, clientOptionalInteger(arguments, "--limit", 20))
		if err != nil {
			return err
		}
	...
	case "bind":
		...
		resolution, err := service.Bind(context.Background(), kind, name, candidate, store)
		if err != nil {
			return err
		}
