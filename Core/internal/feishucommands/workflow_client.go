...
func (call *invocation) runWorkflowClient(dataRoot string, service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	if len(arguments) == 0 {
		return errors.New("missing workflow action")
	}
	switch arguments[0] {
	case "standup-report":
		return call.runStandupWorkflow(dataRoot, service, arguments[1:], write)
	case "meeting-summary":
		return call.runMeetingSummaryWorkflow(dataRoot, service, arguments[1:], write)
	default:
		return errors.New("workflow action must be standup-report or meeting-summary")
	}
}

func (call *invocation) runStandupWorkflow(dataRoot string, service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	...
	calendar, err := call.executeWorkflowRead(service, "calendar.shortcut.agenda", map[string]any{"start": start.Format(time.RFC3339), "end": end.Format(time.RFC3339)})
	if err != nil {
		return err
	}
	tasks, err := call.executeWorkflowRead(service, "task.shortcut.get.my.tasks", map[string]any{"page-limit": 1, "complete": false})
	if err != nil {
		return err
	}
...
func (call *invocation) runMeetingSummaryWorkflow(dataRoot string, service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	...
	if target.Source == "meeting" {
		meetings, err = call.executeWorkflowRead(service, "vc.shortcut.detail", map[string]any{"meeting-ids": strings.Join(target.MeetingIDs, ",")})
	} else {
		minuteLookup, err = call.executeWorkflowRead(service, "minutes.minutes.get", map[string]any{"minute-token": target.MinuteTokens[0]})
	}
	if err != nil {
		return err
	}
	...
	for _, noteID := range noteIDs {
		value, readErr := call.executeWorkflowRead(service, "note.shortcut.detail", map[string]any{"note-id": noteID})
		if readErr != nil {
			return readErr
		}
	...
	if len(minuteTokens) > 0 {
		minutes, err = call.executeWorkflowRead(service, "minutes.shortcut.detail", map[string]any{
	...
	if clientHasFlag(arguments, "--include-transcript") {
		...
		for _, token := range minuteTokens {
			value, readErr := call.executeWorkflowRead(service, "minutes.shortcut.detail", map[string]any{"minute-tokens": token, "transcript": true})
			if readErr != nil {
				return readErr
			}
...
func (call *invocation) executeWorkflowRead(service *feishu.CapabilityService, capabilityID string, input map[string]any) (map[string]any, error) {
	prepared, err := call.prepare(service, call.ctx, capabilityID, input, "workflow")
	if err != nil {
		return nil, err
	}
