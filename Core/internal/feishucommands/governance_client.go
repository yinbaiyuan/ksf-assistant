...
func (call *invocation) runOperationClient(dataRoot string, service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	if len(arguments) == 0 {
	...
	case "prepare":
		...
		result, err := call.prepare(service, call.ctx, arguments[1], input, source)
		if err != nil {
			advice := feishu.CapabilityServiceErrorAdvice(err)
			return write(map[string]any{"status": "rejected", "errorCode": advice.ErrorCode, "nextAction": advice.NextAction})
		}
	...
	case "confirm":
		...
		challengeBytes, err := call.clientPrivateValue(arguments[1:], "--challenge-file")
		if err != nil {
			return err
		}
		result, err := service.Confirm(call.ctx, id, strings.TrimSpace(string(challengeBytes)))
		if err != nil {
...
func (call *invocation) runPolicyClient(service *feishu.CapabilityService, arguments []string, write clientJSONWriter) error {
	if len(arguments) == 0 {
	...
	case "update":
		payload, err := call.clientPayload(arguments[1:])
		if err != nil {
			return err
		}
