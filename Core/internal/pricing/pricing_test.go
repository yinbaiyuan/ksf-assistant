...
func TestCatalogContainsSevenOfficialPlansAndDefault(t *testing.T) {
	catalog := Catalog(nil)
	if catalog.DefaultPlanID != DefaultPlanID {
		t.Fatalf("unexpected default plan: %q", catalog.DefaultPlanID)
	}
	if len(catalog.Plans) != 7 {
		t.Fatalf("expected 7 built-in plans, got %d", len(catalog.Plans))
	}
...
func TestCatalogCapsCustomPlansAndRejectsDuplicateIDsAndLongLabels(t *testing.T) {
	...
	if len(catalog.Plans) != 7+MaxCustomPlans || len(catalog.RejectedCustomPlanIDs) != 3 {
