...
const (
	HostContextFilename = "ksfassistant-host-context-v1.json"
	HostContextProtocol = "ksfassistant-host-context-v1"
...
func (store HostContextStore) Load() (HostContext, error) {
	...
	if missing {
		return HostContext{}, errors.New("KSFAssistant host context is unavailable")
	}
	if err != nil {
...
func (store HostContextStore) SaveKSFRoot(root string) (HostContext, error) {
	...
	if err := privatestore.WriteJSON(store.path, context); err != nil {
		return HostContext{}, err
	}
	return context, nil
}
...
func (context HostContext) Validate() error {
	if context.Protocol != HostContextProtocol || context.SchemaVersion != 1 || context.UpdatedAt.IsZero() {
		return errors.New("invalid KSFAssistant host context")
	}
