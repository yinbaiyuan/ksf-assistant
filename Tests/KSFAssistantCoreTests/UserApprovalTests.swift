...
final class UserApprovalTests: XCTestCase {
    ...
    func testNativeHostIsolationAndLifecycleWiring() throws {
        ...
        XCTAssertTrue(host.contains("本机受管工具请求以你的身份操作飞书"))
        XCTAssertFalse(host.contains("Codex 请求"))
        XCTAssertTrue(host.contains("NSScrollView"))
        XCTAssertTrue(host.contains("text.string = request.details"))
        XCTAssertTrue(host.contains("panel.defaultButtonCell = reject.cell"))
        XCTAssertTrue(host.contains("cancelOperation"))
