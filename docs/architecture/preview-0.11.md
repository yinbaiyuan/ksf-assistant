...
桌面接入已有飞书应用，通过 stdin 传递 App Secret 给官方配置命令。用户授权使用官方 `auth login --json` 的流式设备授权事件：桥保有进程、桌面显示链接/二维码，官方 CLI 持有设备凭据、完成轮询和保存授权。进程最长十分钟，取消与退出会回收，不把设备码放入 argv、状态文件或返回 JSON。旧设备码恢复入口要求重新授权。

本轮仅固定 `default` profile 和 `feishu` 品牌。官方配置目录与受管命令共用；保留已有 Keychain、DPAPI 及业务数据位置，不解密导出旧秘密、不再新建 SDK 专用凭据副本。授权有效、权限齐备、CLI 可用、事件消费者就绪、Desktop 控制可用分别检查；身份不足不自动切换另一身份。

**初始预览的边界：** 初始预览只支持接入已有应用；旧的分离进程创建流程已移除，避免无法回收的授权进程。当前源码新增受管的[首次配置扫码创建流程](feishu-app-registration.md)，不覆盖已有配置，进程可取消并有超时限制；实机验收状态独立记录。官方审核、应用发布及管理员批准不由产品绕过。产品管理接入体验，不声称能限制用户在应用外直接使用 CLI。
