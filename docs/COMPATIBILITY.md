...
| 核心服务 | `codex-usage-core-v2`；Go 原生二进制与宿主架构匹配 | 核心不可用时宿主显示恢复提示，不直接读取 Codex/KSF/飞书数据 |
...
| 飞书服务 | macOS 与 Windows 四个目标均使用包内 Go 服务及固定 `lark-cli 1.0.92`；任务控制要求 `codex-feishu-task-link-v1` response v2 | 飞书状态、测试发送与任务连接独立降级；退出 CodexAssistant 时随核心服务一起停止；不自动跨运行时回退 |
