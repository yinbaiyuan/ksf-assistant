...
每条默认根消息创建独立 Codex Thread，卡内追问和引用回复只续接该卡片绑定的 Thread。默认对话的每个飞书轮次使用独立 App Server 客户端执行，结束后取消订阅并关闭连接，只保留持久 Thread，不继续占用 Codex 写入者；因此用户可随后在 Codex Desktop 中直接输入。项目升级后的飞书后续轮次统一交给 Desktop IPC。默认对话与任务卡都使用最长 1000 字的单行快速输入，并把“发送”放在输入框右侧；默认卡的“关闭卡片”位于次级操作行。进入 JSON 2.0 生命周期后，处理中、完成、失败和关闭状态不再切回旧 schema，也不输出旧卡专用根字段。本轮真实耗时只在终态辅助信息中加粗展示，不再和用户问题或 Codex 回复混写。

Usage Bar 任务连接卡使用两种明确输入方式。日常短消息使用卡内“快速回复”单行输入，主发送按钮与输入框保持同一行；多行文字、图片、文件、音频、视频或富文本直接使用飞书的“回复卡片”发送，引用关系会把内容精确路由到对应任务，不再提供“长文本 / 附件”的选中与等待按钮。标题栏右侧使用飞书原生语义色标签显示“运行中 / 已完成 / 等待回答 / 失败”等主状态。正文首行只保留控制方、Plan、权限、阶段和耗时，终态租约时间显示在第二行；“断连”固定在该信息区右侧并右对齐，不再混入高频操作区。下一轮 Plan/默认模式和停止保留在次级控制行。模式切换只影响后续轮次，并由 Codex Desktop 任务所有者确认后生效；Plan 轮次生成的完整计划会在原卡片的独立“计划”区持续同步，完成后仍保留，不与 Codex 动态回复重复。运行中输入用于修正当前轮，终态输入在同一任务开启下一轮，等待普通问题时作为回答；排队、等待 Desktop、过期和断连状态不显示输入入口。任务连接卡的按钮回调与后台进度不得分别直写飞书：两者统一进入同一任务的串行消息更新队列，常规更新固定优先使用消息 Patch，回调 Token 只作为该次写入失败后的队内兜底，防止旧完成态覆盖新运行态。旧版附件等待动作只负责清理历史状态，不再接管未引用的下一条消息。旧刷新回调继续兼容。飞书移动端点击卡片输入组件是否进入独立编辑页属于客户端行为，卡片布局不承诺改变该交互。
