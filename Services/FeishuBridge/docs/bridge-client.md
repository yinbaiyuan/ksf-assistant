...
每条默认根消息创建独立 Codex Thread，卡内追问和引用回复只续接该卡片绑定的 Thread。默认对话的每个飞书轮次使用独立 App Server 客户端执行，结束后取消订阅并关闭连接，只保留持久 Thread，不继续占用 Codex 写入者；因此用户可随后在 Codex Desktop 中直接输入。项目升级后的飞书后续轮次统一交给 Desktop IPC。默认对话与任务卡都使用最长 1000 字的单行快速输入，并把“发送”放在输入框右侧；默认卡的“关闭卡片”收进顶部原生更多菜单。进入 JSON 2.0 生命周期后，处理中、完成、失败和关闭状态不再切回旧 schema，也不输出旧卡专用根字段。本轮真实耗时只在终态辅助信息中展示：普通对话继续加粗，任务连接卡使用任务名下方的灰色辅助信息，不再和用户问题或 Codex 回复混写。

CodexAssistant 任务连接卡使用两种明确输入方式。日常短消息使用卡内单行输入；多行文字、图片、文件、音频、视频或富文本直接使用飞书的“回复卡片”发送，引用关系会把内容精确路由到对应任务，不再提供“长文本 / 附件”的选中与等待按钮。标题栏右侧使用飞书原生语义色标签显示“运行中 / 已完成 / 等待回答 / 失败”等主状态。标题下首行以加粗主文字显示不重复项目名的任务名，右侧使用红色描边“断开”；长任务名使用自适应列自然换行，按钮保持右上对齐。控制方、Plan、权限、具体阶段、耗时和租约合并为下一行灰色辅助信息；“运行中”“当前进展”等通用阶段不重复显示。任务名与项目名相同时不再重复，辅助信息与“断开”共用首行。运行中与等待回答的表单严格按“停止 → 输入框 → 发送”排列：“停止”使用红色描边并保持独立回调，不提交输入内容；排队和等待 Desktop 没有输入框时，停止按钮位于正文下方。运行中不显示冗余字段标题，输入框占位文案为“补充或修正”；等待普通问题时仍标为“回答 Codex”。终态表单在左侧把“开始新一轮”和默认/Plan 下拉选择紧邻排列，模式和值随输入一次提交，并携带安全的 `intent=new_turn` 意图标记。即使客户端没有回传未改动的模式值，桥也会沿用已保存模式并执行终态复核；状态已变化时拒绝整次提交，不会误当作当前轮补充。新卡不再预先显示独立模式按钮。Plan 轮次生成的完整计划会在原卡片的独立“计划”区持续同步；桥侧 Plan 轮次结束后会在串行锁内按 0、250、750 ms 读取 Desktop 权威快照，首次终态直接进入 `plan_ready`，不同的新轮次不会被旧完成态覆盖。待执行计划把满宽“开始执行”放在计划正文后，并在下方保留“修改计划”输入与紧邻的次级“提交修改”。排队、等待 Desktop、过期和连接已断开状态不显示输入入口。任务连接卡的按钮回调与后台进度不得分别直写飞书：两者统一进入同一任务的串行消息更新队列，常规更新固定优先使用消息 Patch，回调 Token 只作为该次写入失败后的队内兜底，防止旧完成态覆盖新运行态。旧版模式、附件等待和刷新回调继续兼容，但不再由新卡片生成。飞书移动端点击卡片输入组件是否进入独立编辑页属于客户端行为，卡片布局不承诺改变该交互。
