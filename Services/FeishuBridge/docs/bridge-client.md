...
CodexAssistant 任务连接卡使用两种明确输入方式。日常短消息使用卡内单行输入，提交按钮始终紧邻输入框；多行文字、图片、文件、音频、视频或富文本直接使用飞书的“回复卡片”发送，引用关系会把内容精确路由到对应任务，不再提供“长文本 / 附件”的选中与等待按钮。标题栏右侧使用飞书原生语义色标签显示“运行中 / 已完成 / 等待回答 / 失败”等主状态。正文首行只保留控制方、Plan、权限、阶段和耗时，终态租约时间显示在第二行；运行中、等待回答、排队和等待 Desktop 时把“停止本轮”作为该状态的危险文字操作放在首行右侧，低频“断开连接”收进同一区域的原生更多菜单。运行中输入标为“补充当前轮”，等待普通问题时标为“回答 Codex”；终态表单以“开始新一轮”及默认/Plan 下拉选择开头，模式和值随输入一次提交，并携带安全的 `intent=new_turn` 意图标记。即使客户端没有回传未改动的模式值，桥也会沿用已保存模式并执行终态复核；状态已变化时拒绝整次提交，不会误当作当前轮补充。新卡不再预先显示独立模式按钮。Plan 轮次生成的完整计划会在原卡片的独立“计划”区持续同步，完成后仍保留，不与 Codex 动态回复重复；待执行计划把满宽“开始执行”放在计划正文后，并在下方保留“修改计划”输入与紧邻的次级“提交修改”。排队、等待 Desktop、过期和断连状态不显示输入入口。任务连接卡的按钮回调与后台进度不得分别直写飞书：两者统一进入同一任务的串行消息更新队列，常规更新固定优先使用消息 Patch，回调 Token 只作为该次写入失败后的队内兜底，防止旧完成态覆盖新运行态。旧版模式、附件等待和刷新回调继续兼容，但不再由新卡片生成。飞书移动端点击卡片输入组件是否进入独立编辑页属于客户端行为，卡片布局不承诺改变该交互。
