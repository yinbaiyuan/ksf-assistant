...
Shared Core 在启动桥前原子写入私有 `codexassistant-host-context-v1`，并以 `CODEX_USAGE_BAR_HOST_CONTEXT` 只传文件位置。上下文只含 `not_configured / ready / invalid` KSF 状态和 `ready` 时的规范绝对根目录，不含凭据、正文、任务或消息标识。每次新根消息重新读取该文件；`ready` 在 KSF 根创建真实任务但不伪造项目投影，`not_configured` 使用托管通用工作区，`invalid` 拒绝创建线程。卡片与引用回复继续使用 schema 1 绑定中保存的原 Thread 与 cwd，因此更新不迁移历史会话。
...
- 普通飞书任务由独立 app-server 持久化为标准 Thread，CodexAssistant 通过 Shared Core 正常发现；已连接 Desktop 任务的实时同步只依赖私有 Desktop IPC 与本地 rollout，不依赖 proxy 或 daemon。
