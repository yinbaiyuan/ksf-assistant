...
初始化凭据时，默认复用既有飞书应用，不自动创建新的 CLI 应用。二维码 OAuth 只能授权已经配置到本机的应用，不能把既有应用的 App Secret 从飞书平台取回；因此复用“尹超Codex”这类既有机器人时，先通过本机安全导入、既有 `lark-cli` profile 或 Agent 绑定让当前 Windows 用户拥有应用配置。
